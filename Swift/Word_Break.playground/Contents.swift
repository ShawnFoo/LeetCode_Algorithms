...
/**
 ...
 思路: 假定F(n) = true意味着0..<n的字符串可拆, 可推导出 F(n) = F(i) + wordDict.contains(String[n-i..<n]) 成立
 
 即String[0..<i]可拆, String[i..<n]包含在字典内 等价于 String[0..<n]可拆
 */
class SolutionDP {
    func wordBreak(_ s: String, _ wordDict: [String]) -> Bool {
        ...
        let chars = Array(s)
        var maxLength = 0 // 求出字典中单词的最大长度. 用于过滤不必要的查找
        ...
        var canBreakIndexs = Array<Bool>(repeating: false, count: s.count + 1)
        canBreakIndexs[0] = true
        for i in 1...s.count {
            for j in (0...i-1).reversed() {
                if canBreakIndexs[j] && i-j <= maxLength { // j之前可拆
                    let word = String(chars[j..<i])
                    if wordSet.contains(word) { // 若j..<i在字典中可拆. 则0...i-1可拆
                        canBreakIndexs[i] = true
                        break
                    }
                }
            }
        }
        return canBreakIndexs[s.count]
    }
}
